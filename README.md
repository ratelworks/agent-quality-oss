...
채택 표준: JSON-LD 1.1 · RDF/OWL · SKOS · PROV-O · schema.org · SHACL. IRI 네임스페이스(`cc:` 건설 공통 · `quality:` · `kcs:`/`kds:`/`ks:`)는 자매 프로젝트 [`agent-safety-oss`](https://github.com/ratelworks/agent-safety-oss)(`safety:`)와 **같은 IRI 공간**을 공유하므로, 안전·품질 그래프를 합쳐도 LLM 이 하나의 건설 도메인으로 관계 추론을 이어갈 수 있습니다. 런타임은 무거운 그래프 DB 없이 로컬 파일 + MCP 도구로 동작합니다.
