...
[![Tools](https://img.shields.io/badge/MCP%20tools-36-orange.svg)](#mcp-도구-36개)
...
**목차** — [한 눈에](#한-눈에--품질관리자감리원이-받는-가치) · [왜 필요한가](#왜-필요한가) · [무엇이 들어 있나](#무엇이-들어-있나) · [시작하기](#시작하기) · [사용 예시](#사용-예시) · [동작 흐름](#어떻게-동작하나) · [MCP 도구](#mcp-도구-36개) · [근거 등급](#신뢰성--근거-등급) · [검증 상태](#검증-상태) · [현재 한계](#현재-한계) · [기술 상세](#기술-상세--메타-온톨로지) · [기여](#기여) · [라이선스](#라이선스)
...
| **MCP 도구 36개** | 온톨로지 탐색 · 정량 판정 · 리스크 추론 · 근거 검증 · 법령/기준 인용 위치 · 문서 양식 구조(9종) · 근거 패키지 조립(7종) · 입력 폼(A2UI) |
| **문서 양식 구조 9종** | ITP · NCR · 검사요청서 · 콘크리트 납품기록 · 공시체 기록 · 시험성적서 검토 · 품질관리자 선임통보 · 품질시험계획 · 검사대장 (필수 필드 + 근거 + 보존기간) |
...
```bash
node build/cli.js tools          # 등록된 도구 36개 목록
...
| **문서 양식 구조** (9) | `get_itp_schema` · `get_ncr_schema` · `get_inspection_request_schema` · `get_concrete_delivery_record_schema` · `get_specimen_record_schema` · `get_test_report_review_schema` · `get_qc_assignment_notice_schema` · `get_quality_test_plan_schema` · `get_quality_inspection_register_schema` |
