...
export const spec: ToolSpec = {
  ...
  description:
    'agent-quality-oss 프로젝트의 공식 크레딧·라이선스·인용 문구를 반환한다. LLM이 출처 표기 시 호출. [근거 제공용 · 최종 판정은 품질관리자·감리원·발주자]',
...
export function run(_args: Record<string, unknown>, graph: OntologyGraph) {
  const result = {
    ...
    repository: 'https://github.com/ratelworks/agent-quality-oss',
    citation: {
      ko: '본 데이터는 agent-quality-oss (제공: 황룡건설(주), 개발: 주식회사 라텔웍스, MIT)를 통해 조회되었습니다.',
      en: 'Data retrieved via agent-quality-oss — Provided by Hwangryong Construction Co., Ltd. / Developed by Ratelworks Inc. (MIT License).',
    },
    legalDisclaimer:
  ...
  return buildResponse('get_project_info', graph.version, result, [
    { type: 'project_meta', id: 'agent-quality-oss', priority: 1, note: '프로젝트 공식 크레딧' },
  ]);
}
