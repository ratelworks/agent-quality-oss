// ─────────────────────────────────────────────────────────────────────────────
// mcp/http.ts — HTTP 어댑터. node:http RequestListener 시그니처로 반환.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (pathname.startsWith("/mcp/tools/") && method === "POST") {
        ...
        await runToolHttp(name, await readJson(req), graph, res);
        return;
      }
      ...
      if (pathname === "/api" && method === "POST") {
        ...
        await runToolHttp(action, body?.params ?? null, graph, res);
        return;
      }

      send(res, 404, { error: `not found: ${method} ${pathname}` });
    } catch (err: unknown) {
      const { status, payload } = toHttpErrorPayload(err);
...
async function runToolHttp(
  ...
  if (!tool) {
    send(res, 404, { ok: false, error: `unknown tool: ${name}` });
    return;
  }
  try {
