...
// ── 상수 ────────────────────────────────────────────────────
const REPO_URL = "https://github.com/ratelworks/agent-quality-oss";
...
Node.js 22+ 환경에서 (소스 실행):

\`\`\`bash
git clone ${REPO_URL}.git
cd agent-quality-oss && npm install && npm run build && npm run start:stdio
\`\`\`

Claude Desktop 등 MCP 지원 앱 등록 방법은 [README](${REPO_URL}#readme) 를 참조하십시오.
${breakingLine}
---

- **제공**: 황룡건설(주) · **개발**: 주식회사 라텔웍스 (Ratelworks Inc.)
